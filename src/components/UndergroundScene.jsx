...
function Root({ book, depth, index, onTap, isWatered }) {
  ...
  return (
    <g transform={`translate(${x}, 0)`} onClick={() => onTap(book)} style={{ cursor: 'pointer' }}>
      {isWatered && (
      ...
      <rect x="4" y="0" width="20" height={rootH} rx="3" fill={genreData.color} opacity="0.85" />
      ...
      {isWatered && <text x="14" y="-4" textAnchor="middle" fontSize="8">✨</text>}
      {book.isFavorite && <text x="14" y={rootH * 0.45} textAnchor="middle" fontSize="9">⭐</text>}
    </g>
  );
}
    ...
    <div className="relative w-full overflow-hidden" style={{ minHeight: 200, background: 'linear-gradient(to bottom, #92400e 0%, #78350f 30%, #451a03 100%)' }}>
      <div className="absolute inset-0 opacity-20">
        {Array.from({ length: 30 }).map((_, i) => (
          <div key={i} className="absolute rounded-full bg-amber-900" style={{ width: 3 + (i % 4), height: 3 + (i % 3), left: `${(i * 37) % 100}%`, top: `${(i * 23 + 10) % 100}%` }} />
        ))}
      </div>
      <div className="absolute top-2 left-3"><span className="text-xs text-amber-300 font-semibold opacity-70">🌍 地下 — 知識の根</span></div>
      ...
      {readBooks.length === 0 ? (
        ...
        <div className="pt-8 pb-4 px-2 overflow-x-auto">
          <svg viewBox={`0 0 380 ${Math.max(140, Math.ceil(sortedBooks.length / 7) * 100 + 40)}`} className="w-full" style={{ minHeight: 180 }}>
            {sortedBooks.map((book, i) => (
              <Root key={book.id} book={book} depth={i} index={i} totalRoots={sortedBooks.length} onTap={setSelectedBook} isWatered={book.watered} />
            ))}
          </svg>
        </div>
      )}

...
const COLS = 5;
const COL_W = 64;
const COV_H = 36; // cover zone above ground line
const ROW_H = 148; // total row height
...
function RootSVG({ book, index, onTap }) {
  ...
  const b1mid = [(b1x0 + b1x1) / 2 - 5, (b1y0 + b1y1) / 2];
  ...
  const b2mid = [(b2x0 + b2x1) / 2 + 5, (b2y0 + b2y1) / 2];
  ...
  return (
    <g onClick={() => onTap(book)} style={{ cursor: 'pointer' }}>
      ...
      {/* Primary branches */}
      <path d={`M ${b1x0} ${b1y0} Q ${b1mid[0]} ${b1mid[1]} ${b1x1} ${b1y1}`}
            stroke={c} strokeWidth="1.8" fill="none" strokeLinecap="round" opacity={0.82} />
      <path d={`M ${b2x0} ${b2y0} Q ${b2mid[0]} ${b2mid[1]} ${b2x1} ${b2y1}`}
            stroke={c} strokeWidth="1.8" fill="none" strokeLinecap="round" opacity={0.82} />
      ...
      {/* Cover thumbnail or genre circle */}
      {book.coverUrl ? (
        <>
          <defs>
            <clipPath id={clipId}>

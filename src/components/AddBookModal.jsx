        ...
        <form onSubmit={handleSubmit} className="px-5 pb-6 space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              {Object.entries(GENRES).map(([key, g]) => (
                <button
                  key={key}
                  type="button"
                  onClick={() => set('genre', key)}
                  className="px-3 py-1.5 rounded-full text-xs font-semibold border-2 transition-all"
                  style={{
                    borderColor: form.genre === key ? g.color : '#e5e7eb',
                    background: form.genre === key ? g.color + '22' : 'white',
                    color: form.genre === key ? g.darkColor : '#6b7280',
                  }}
                >
                  {g.emoji} {g.label}
                </button>
          ...
          <div className="flex gap-3">
            <button
              ...
              style={{
                borderColor: form.isFavorite ? '#fbbf24' : '#e5e7eb',
                background: form.isFavorite ? '#fef3c7' : 'white',
                color: form.isFavorite ? '#92400e' : '#6b7280',
              }}
            >
              ⭐ お気に入り
            </button>
            <button
              type="button"
              onClick={() => set('isUnread', !form.isUnread)}
              className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm border-2 transition-all"
              style={{
                borderColor: form.isUnread ? '#a78bfa' : '#e5e7eb',
                background: form.isUnread ? '#ede9fe' : 'white',
                color: form.isUnread ? '#5b21b6' : '#6b7280',
              }}
            >
              📦 積読
            </button>
